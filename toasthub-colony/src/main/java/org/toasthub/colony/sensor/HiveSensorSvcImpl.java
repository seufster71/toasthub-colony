...
@Service("CCHiveSensorSvc")
public class HiveSensorSvcImpl implements HiveSensorSvc, ServiceProcessor {

	@Autowired
	@Qualifier("CCHiveSensorDao")
	HiveSensorDao hiveSensorDao;
	...
	public void process(RestRequest request, RestResponse response) {
		...
		case "SAVE":
			if (!request.containsParam(PrefCacheUtil.PREFFORMKEYS)) {
				List<String> forms =  new ArrayList<String>(Arrays.asList("CC_HIVESENSOR_FORM"));
	...
	public void items(RestRequest request, RestResponse response) {
		try {
			hiveSensorDao.items(request, response);
	...
	public void itemCount(RestRequest request, RestResponse response) {
		try {
			hiveSensorDao.itemCount(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
		}
	...
	public void delete(RestRequest request, RestResponse response) {
		try {
			hiveSensorDao.delete(request, response);
	...
	public void item(RestRequest request, RestResponse response) {
		try {
			hiveSensorDao.item(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
		}
	...
	public void save(RestRequest request, RestResponse response) {
		try {
			...
			if (inputList.containsKey(GlobalConstant.ITEMID) && inputList.get(GlobalConstant.ITEMID) != null && !"".equals(inputList.get(GlobalConstant.ITEMID))) {
				...
				HiveSensor hiveSensor = new HiveSensor();
				hiveSensor.setActive(true);
				hiveSensor.setArchive(false);
				hiveSensor.setLocked(false);
				request.addParam(GlobalConstant.ITEM, hiveSensor);
			}
			// marshall
			...
			// save
			hiveSensorDao.save(request, response);
