...
@Service("CCOrganizationSvc")
public class OrganizationSvcImpl implements OrganizationSvc, ServiceProcessor {

	@Autowired
	@Qualifier("CCOrganizationDao")
	...
	public void items(RestRequest request, RestResponse response) {
		try {
			organizationDao.items(request, response);
	...
	public void itemCount(RestRequest request, RestResponse response) {
		try {
			organizationDao.itemCount(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
		}
	...
	public void delete(RestRequest request, RestResponse response) {
		try {
			organizationDao.delete(request, response);
	...
	public void item(RestRequest request, RestResponse response) {
		try {
			organizationDao.item(request, response);
		} catch (Exception e) {
			utilSvc.addStatus(RestResponse.ERROR, RestResponse.EXECUTIONFAILED, prefCacheUtil.getPrefText("GLOBAL_SERVICE", "GLOBAL_SERVICE_EXECUTION_FAIL",prefCacheUtil.getLang(request)), response);
			e.printStackTrace();
		}
	...
	public void save(RestRequest request, RestResponse response) {
		try {
			...
			if (inputList.containsKey(GlobalConstant.ITEMID) && inputList.get(GlobalConstant.ITEMID) != null && !"".equals(inputList.get(GlobalConstant.ITEMID))) {
				...
				organization.setArchive(false);
				organization.setLocked(false);
				organization.setUserId(((MyUserPrincipal) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUser().getId());
				request.addParam(GlobalConstant.ITEM, organization);
			}
			// marshall
			utilSvc.marshallFields(request, response);
			
			// save
			organizationDao.save(request, response);
